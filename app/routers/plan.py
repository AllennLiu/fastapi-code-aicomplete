...
class PrerequisiteTable(BaseModel):
    tools: set[str] = Field(default_factory=set, description='predict prerequisite tools')
...
class PrerequisiteJob(Prerequisite):
...
job_default = PrerequisiteJob(bkm_uuid='', prerequisite='')

async def cleanup_model_core_dump() -> None:
    """Cleanup the core dump files _(core.*)_ which is generated by
    model inference error, this is use to avoid the model's memory
...
async def predict_prerequisites(
    pipeline: Pipeline, prerequisite: Prerequisite | PrerequisiteJob, maximum_retry: int = 3
) -> PrerequisiteTable:
    ...
    chat = PlanChat(prompt=f'{prerequisite.prerequisite}\nReturn a list[str] in JSON format.')
    ...
    for remain in range(maximum_retry - 1, -1, -1):
        try:
            ...
            tools = cast(set[str], json.loads(content_without_md))
            print_process(f'response:\n{colorama.Fore.GREEN}{tools}')
            return PrerequisiteTable(tools=tools, elapsed_time=elapsed_time)
        except json.decoder.JSONDecodeError:
            print(f'{error_msg}\nException error: Invalid Jsonify Content')
            messages[-1].content = f'{prerequisite.prerequisite}\nReturn the format of JSON list[str] only.'
    ...
    return PrerequisiteTable(elapsed_time=elapsed_time)

async def update_prerequisites_model(
    name: str, models: Iterable[Prerequisite | PrerequisiteJob], plan_uuid: str
) -> None:
    """Update the model's prerequisites data within an array then
    save it to ``Redis`` with ``hset`` method.

    Args:
        name (str): which :func:`~hset` name to be updated.
        models (Iterable[Prerequisite | PrerequisiteJob]): \
            a list of prerequisite or prerequisite job.
        plan_uuid (str): which key of ``ARES plan UUID`` to \
            be queried.
    """
...
async def get_prerequisites_todo(plan_uuid: str) -> List[Prerequisite]:
    """Asynchronous to fetch all case's prerequisites by ARES plan
    UUID with ``Mongodb``, use for requests todo list.

    Args:
        plan_uuid (str): which ``ARES plan UUID`` to be queried.

    Returns:
        tuple[List[Prerequisite], str]: a tuple with list \
            of case UUID + prerequisite and plan UUID.
    """
    pipeline = [
        {
            "$lookup": {
...
async def create_prerequisites_job(
    ...
    """Asynchronous to create todo job list by fetched todo list
    and check if the prerequisite of job is not equal as todo,
    then update the todo job's completed status to ``False``.
    ...
    async with RedisAsynchronous(**settings.redis.model_dump(), decode_responses=True).connect() as r:
        query = await r.hget(f'model-prerequisites-job', plan_uuid)
...
async def predict_prerequisites_job(
    ...
    then update tools and elapsed time to ``model-prerequisites-table``
    in realtime during the prediction loop. finally, update the
    ``model-prerequisites-job`` to indicate the job is completed.
    ...
    Returns:
        Dict[str, Dict[str, List[str] | float]]: a dictionary \
            of tools and elapsed time.
    """
    async with RedisAsynchronous(**settings.redis.model_dump(), decode_responses=True).connect() as r:
        ...
        for job in itertools.filterfalse(operator.attrgetter('completed'), jobs):
            ...
            predict_result = await predict_prerequisites(pipeline, job)
            tables[job.bkm_uuid] |= predict_result.model_dump(mode='json')
            job_save = next((e for e in jobs if e.bkm_uuid == job.bkm_uuid), job_default)
            job_save.completed = True
            job_save.rate = rate
            await r.hset('model-prerequisites-table', plan_uuid, json.dumps(tables))
            await update_prerequisites_model('model-prerequisites-job', jobs, plan_uuid)
    return tables

def summary_tools(table: Dict[str, Dict[str, List[str] | float]]) -> tuple[SummaryResult, float]:
    """Processing structured response fulfill customize requirement
    and summarize each of the prerequisite elapsed time.

    Args:
        table (Dict[str, Dict[str, List[str] | float]]): \
            a dictionary of tools and elapsed time.
    ...
    elapsed_time = sum(map(operator.itemgetter('elapsed_time'), table.values()))
    tools = itertools.chain.from_iterable(map(operator.itemgetter('tools'), table.values()))
    for tool in cast(Iterable[str], tools):
        if bool(pattern_url.match(tool)):
            ...
            if 'inventec.com' in url.netloc and condition:
...
async def plan_prerequisite_progress(plan_id: Annotated[int, PARAM_PATH_PLAN_ID]) -> JSONResponse:
    """Summarize to calculate the progress of plan's prerequisite
    jobs for monitoring the progress of the prediction.
    """
...
async def remove_prerequisites_job(plan_id: Annotated[int, PARAM_PATH_PLAN_ID]) -> JSONResponse:
    """Remove the prerequisite job and todo list by ARES plan UUID."""
    plan_uuid = await get_plan_uuid_by_id(plan_id)
    async with RedisAsynchronous(**settings.redis.model_dump(), decode_responses=True).connect() as r:
        for key in 'job', 'todo':
